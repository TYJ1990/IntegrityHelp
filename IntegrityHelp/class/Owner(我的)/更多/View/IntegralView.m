...
- (void)getChartsWithPropetyValueArray:(NSArray *)valueArr nameArray:(NSArray *)nameArr{
    ...
    [_kEchartView setOption:[IntegralView standardPieOptionPropetyWithValueArray:valueArr nameArray:nameArr]];
    [_kEchartView loadEcharts];
}
...
+ (PYOption *)standardPieOptionPropetyWithValueArray:(NSArray *)valueArr nameArray:(NSArray *)nameArr{
    return [PYOption initPYOptionWithBlock:^(PYOption *option) {
        option.tooltipEqual([PYTooltip initPYTooltipWithBlock:^(PYTooltip *tooltip) {
            tooltip.triggerEqual(PYTooltipTriggerItem)
            .formatterEqual(@"{a} <br/>{b} : {c} ({d}%)");
        ...
        .addSeries([PYPieSeries initPYPieSeriesWithBlock:^(PYPieSeries *series) {
            ...
            .centerEqual(@[@"72%",@"50%"])
            .typeEqual(PYSeriesTypePie)
            .dataEqual(@[@{@"value":valueArr[0],@"name":nameArr[0]},@{@"value":valueArr[1],@"name":nameArr[1]},@{@"value":valueArr[2],@"name":nameArr[2]},@{@"value":valueArr[3],@"name":nameArr[3]}])
            .itemStyleEqual([PYItemStyle initPYItemStyleWithBlock:^(PYItemStyle *itemStyle) {
                itemStyle.normalEqual([PYItemStyleProp initPYItemStylePropWithBlock:^(PYItemStyleProp *itemStyleProp){
                    itemStyleProp.labelEqual([PYLabel initPYLabelWithBlock:^(PYLabel *label) {
                        label.showEqual(NO);
