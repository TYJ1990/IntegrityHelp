...
- (void)setAssetsGroups:(NSArray *)assetsGroups{
    if (_assetsGroups != assetsGroups) {
        ...
        self.tableView.top = self.height - rowCount*kHeightAssetsGroupCell - 44;
//        self.touchButton.top = self.tableView.bottom;
        self.touchButton.height = self.height - self.tableView.height;
        [self.tableView reloadData];
    }
}
...
- (UITableView *)tableView{
    if (!_tableView) {
        _tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, self.height - 4*kHeightAssetsGroupCell, self.width, 4*kHeightAssetsGroupCell) style:UITableViewStylePlain];
...
- (UIButton *)touchButton{
    if (!_touchButton) {
        ...
        _touchButton.frame = CGRectMake(0, 0, self.width, self.height-self.tableView.height);
