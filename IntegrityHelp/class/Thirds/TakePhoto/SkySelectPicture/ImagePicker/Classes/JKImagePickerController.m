...
- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view. 249,158,43 previewPhotoesSelected
...
//    self.title = @"选择照片";
    ...
    self.title = [NSString stringWithFormat:@"选择图片(%ld/%ld)",(unsigned long)self.selectedAssetArray.count,(unsigned long)self.maximumNumberOfSelection];
//    self.navigationItem.titleView = self.titleButton;
    
    self.preBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    self.preBtn.frame = CGRectMake(0, 0, 50, 30);
    [self.preBtn setTitle:@"完成" forState:UIControlStateNormal];
    self.preBtn.titleLabel.font = [UIFont boldSystemFontOfSize:15];
    [self.preBtn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    [self.preBtn setTitleColor:[JKUtil getColor:@"828689"] forState:UIControlStateDisabled];
    [self.preBtn addTarget:self action:@selector(finishPhotoDidSelected) forControlEvents:UIControlEventTouchUpInside];
    UIBarButtonItem *preItem = [[UIBarButtonItem alloc] initWithCustomView:self.preBtn];
...
//        [cancelBtn setTitle:@"取消" forState:UIControlStateNormal];
        [cancelBtn setImage:[UIImage imageNamed:@"back_white"] forState:UIControlStateNormal];
        [cancelBtn setImageEdgeInsets:UIEdgeInsetsMake(0, -50, 0, 0)];
...
- (void)finishPhoto{
    if ([_delegate respondsToSelector:@selector(imagePickerController:didPhoto:)]) {
        [_delegate imagePickerController:self didPhoto:[self.selectedAssetArray lastObject]];
    }
}
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info{
    ...
    [[PhotoAlbumManager sharedManager] saveImage:image
                                 ...
                                 completionBlock:^(ALAsset *asset, NSError *error) {
                                     if (error == nil && asset) {
                                         NSURL *assetURL = [asset valueForProperty:ALAssetPropertyAssetURL];
//                                         [weakSelf addAssetsObject:assetURL];
                                         
                                         NSURL *groupURL = [weakSelf.selectAssetsGroup valueForProperty:ALAssetsGroupPropertyURL];
                                         NSString *groupID = [weakSelf.selectAssetsGroup valueForProperty:ALAssetsGroupPropertyPersistentID];
                                         ...
                                         [weakSelf.selectedAssetArray addObject:asset];
                                         [weakSelf.assetsGroupsView addAssetSelected:asset];
                                         
                                         [weakSelf finishPhoto];
...
//    self.finishLabel.text = [NSString stringWithFormat:@"",self.selectedAssetArray.count];
    self.title = [NSString stringWithFormat:@"选择图片(%ld/%ld)",(unsigned long)self.selectedAssetArray.count,(unsigned long)self.maximumNumberOfSelection];
...
- (UIButton *)titleButton{
    if (!_titleButton) {
        ...
        _titleButton.frame = CGRectMake(0, 7, 130, 30);
...
- (UIButton *)touchButton{
    if (!_touchButton) {
        ...
        _touchButton.frame = CGRectMake(0, self.view.height - 44+64, self.view.width, 44);
...
//        [_finishButton setBackgroundImage:[JKUtil stretchImage:[UIImage imageNamed:@"picker_button_orange_highlighted"] capInsets:UIEdgeInsetsMake(5, 5, 5, 5) resizingMode:UIImageResizingModeStretch] forState:UIControlStateHighlighted];
        ...
        _finishButton.centerY = 22;
        _finishButton.hidden = YES;
        [_finishButton addTarget:self action:@selector(previewPhotoesSelected) forControlEvents:UIControlEventTouchUpInside];
        ...
        [_toolbar addSubview:self.titleButton];
